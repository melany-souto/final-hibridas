...
export async function loadBoard(req, res, next) {
    ...
    // console.log("ID recibido en loadBoard ULTIMO:", boardId);  

    try {
        const board = await service.getBoardById(boardId);
        //  console.log("Board encontradoOOO:", board);
        if (!board) {
            return res.status(404).json({ mensaje: "Tablero no encontrado" });
            console.log("No se encontró el boardDDDDD");
        }

        req.board = board;
        next();
    } catch (error) {
        // console.log("Error en loadBoardDDDDD:", error);
